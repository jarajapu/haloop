...
class MapOutputFile {
	 ...
	 *            a map task id
	 */
	public Path getOutputFile(TaskAttemptID mapTaskId, int pass)
			throws IOException {
	 ...
	 *            a map task id
	 */
	public Path getOutputIndexFile(TaskAttemptID mapTaskId, int currentIteration)
			throws IOException {
	 ...
	 *            the number
	 */
	public Path getSpillFile(TaskAttemptID mapTaskId, int spillNumber, int pass)
			throws IOException {
	...
	public Path getSpillIndexFile(TaskAttemptID mapTaskId, int spillNumber,
		...
		return lDirAlloc.getLocalPathToRead(TaskTracker
				.getIntermediateOutputDir(jobId.toString(), mapTaskId
						.toString())
				+ "/i" + pass + "/spill" + spillNumber + ".out.index", conf);
	}
	...
	public Path getSpillIndexFileForWrite(TaskAttemptID mapTaskId,
			int spillNumber, long size, int pass) throws IOException {
	 ...
	 *            a reduce task id
	 */
	public Path getInputFile(int mapId, TaskAttemptID reduceTaskId, int pass)
			throws IOException {
		return lDirAlloc.getLocalPathToRead(TaskTracker
				.getIntermediateOutputDir(jobId.toString(), reduceTaskId
						.toString())
				+ "/i" + pass + "/map_" + mapId + ".out", conf);
	}
	...
	public Path getInputFileForWrite(TaskID mapId, TaskAttemptID reduceTaskId,
		...
		return lDirAlloc.getLocalPathForWrite(TaskTracker
				.getIntermediateOutputDir(jobId.toString(), reduceTaskId
						.toString())
				+ "/i" + pass + "/map_" + mapId.getId() + ".out", size, conf);
	}
	 ...
	 */
	public Path getMapCacheFileForWrite(TaskAttemptID reduceTaskId, long size,
			int pass) throws IOException {
		return lDirAlloc.getLocalPathForWrite(TaskTracker
				.getIntermediateOutputDir(jobId.toString(), reduceTaskId
						.toString())
